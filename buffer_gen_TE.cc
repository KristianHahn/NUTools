      ...
      while(ievtinner[ifile] <= ievt && !eofinner[ifile]) {
        if(getline(*stubinnerfiles[ifile], line)) {
          if(line.find("Event") != string::npos) {
            ...
            unsigned long stubdata = strtoul(split(line,' ').back().c_str(),NULL,0);
      ...
      while(ievtouter[ifile] <= ievt && !eofouter[ifile]) {
        if(getline(*stubouterfiles[ifile], line)) {
          if(line.find("Event") != string::npos) {
            ...
            unsigned long zbin     = strtoul(tokens.front().c_str(),NULL,0);
            unsigned long stubdata = strtoul(tokens.back().c_str(),NULL,0);
            ...
            // update header word with number of stubs
            unsigned long nstubouterZ_word = 0;
